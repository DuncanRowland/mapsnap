...
class _WebkitRendererHelper(QObject):
    ...
    def render(self, res):
        ...
        #Duncan
        timeoutHack = time.time() + 10 #Max 10 second load time
        #print("1"+str(self._page.mainFrame().findFirstElement('#loaded').toPlainText()))
        while time.time() <= timeoutHack and not str(self._page.mainFrame().findFirstElement('#loaded').toPlainText()).startswith("DONE"):
        #   print("2"+str(self._page.mainFrame().findFirstElement('#loaded').toPlainText()))
           if QApplication.hasPendingEvents():
              QApplication.processEvents()
        #print("3"+str(self._page.mainFrame().findFirstElement('#loaded').toPlainText()))
